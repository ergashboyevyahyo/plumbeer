    ...
    const ctx = gsap.context(() => {
      ...
      if (!prefersReduced) {
        ...
        tl.to(textWrapRef.current, { opacity: 0, scale: 0.9, y: -30, duration: 0.6, ease: 'none' }, 0)
          ...
          .to(scrimRef.current, { opacity: 0.55, duration: 1, ease: 'none' }, 0)
      }
    })
      ...
      >
        ...
        >
          ...
          >
            ...
            <div
              ref={scrimRef}
              className={`absolute inset-0 bg-black ${reduced ? 'opacity-55' : 'opacity-40'}`}
            />
          </div>

          <div ref={textWrapRef} className="relative z-10 flex flex-col items-center px-4 text-center">
            <div className="mb-6 flex items-center gap-3 text-xs uppercase tracking-[0.25em] text-[var(--color-inverse)]/70">
              <span className="h-px w-8 bg-[var(--color-inverse)]/70" />
              {badge}
            </div>
            ...
            <SplitHeading
              ...
              delay={0.6}
              trigger={false}
              className="max-w-5xl text-[clamp(2.5rem,6.5vw,6rem)] leading-[1.15] text-[var(--color-inverse)]"
            />
          </div>
        </div>
      </section>
